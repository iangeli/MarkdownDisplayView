...
final class HistoryMDViewController: UIViewController {
    ...
    private var preparedContents: [Int: MarkdownPreparedContent] = [:]
    private var preparedContentWidth: CGFloat = 0
    private var prepareGeneration = UUID()
    private let prepareQueue = DispatchQueue(label: "com.markdown.history.prepare", qos: .userInitiated)
    ...
    override func viewDidLayoutSubviews() {
        ...
        prepareHistoryMarkdown(width: width)
    }
    ...
    private func prepareMessages() {
        ...
        prepareHistoryMarkdown(width: markdownContentWidth())
    }
    ...
    private func prepareHistoryMarkdown(width: CGFloat) {
        guard width > 1 else { return }
        let generation = UUID()
        ...
        prepareQueue.async { [weak self] in
            var preparedByRow: [Int: MarkdownPreparedContent] = [:]
            var heightsByRow: [Int: CGFloat] = [:]

            for (index, markdown) in sourceMessages.enumerated() {
                let renderer = MarkdownRenderer(containerWidth: width)
                let prepared = renderer.prepare(markdown)
                preparedByRow[index] = prepared

                let extraPadding = index == 0 ? firstExtraPadding : 0
                heightsByRow[index] = prepared.estimatedTotalHeight + verticalPadding + extraPadding
            }

            DispatchQueue.main.async { [weak self] in
                guard let self, self.prepareGeneration == generation else { return }
                self.preparedContents = preparedByRow
                self.cachedHeights = heightsByRow
...
extension HistoryMDViewController: UITableViewDataSource, UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let markdown = messages[safe: indexPath.row] ?? ""
        if let prepared = preparedContents[indexPath.row] {
            cell.configure(preparedContent: prepared)
        } else {
            cell.configure(markdown: markdown)
        }
...
final class MarkdownHistoryCell: UITableViewCell {
    ...
    func configure(preparedContent: MarkdownPreparedContent) {
        renderToken = UUID()
